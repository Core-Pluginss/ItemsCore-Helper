...
const STAT_SCHEMA = {
  ...
  attachingToItems:
    "An item references a stat by its name; the per-item value is set in the editor (/itemeditor <item> -> Stats). Item lore renders each active stat as fancyName + ' ' + fancyValue, with %value% replaced by the effective value (base + per-item + reforge bonuses). The {stats} placeholder renders the whole stat block live.",
}
...
const COMMANDS = [
  { command: "/ic", usage: "/ic", description: "Open the custom items menu: browse, create, edit, give, set recipe, and delete items.", permission: "itemscore.admingui", aliases: ["/itemscore"] },
  ...
  { command: "/ic reload", usage: "/ic reload [items|stats] [name]", description: "Reload from disk. No arguments reloads everything. A category (items or stats) reloads all of that category. Adding a name after the category reloads just that one item or stat. Backward compatible: /ic reload <itemName> still reloads a single item.", permission: "itemscore.admingui" },
...
const ADDON_COMMANDS = [
  { command: "/reforges", usage: "/reforges", description: "ReforgesCore addon: manage reforges (create, edit stats, set prices).", requires: "ReforgesCore addon" },
...
module.exports = { loadManifest, buildIndex, searchMethods, getMethod, validateItem, itemSchema, generateItemTemplate, resolveManifestPath, statSchema, validateStat, STAT_SCHEMA, commandList, COMMANDS }
